...
void processMagnetometerMeasurements(const uint8_t *buffer)
{
    ...
    if (buffer[7] & (1 << HMC5883L_STATUS_READY_BIT)) {
        ...
        sensorData.mag.z = (float)headingx / MAG_GAUSS_PER_LSB; //to gauss
        sensorData.mag.y = (float)headingy / MAG_GAUSS_PER_LSB;
        sensorData.mag.x = (float)headingz / MAG_GAUSS_PER_LSB;
...
// X->Z, Z->X  
    ...
    accelRaw.z = (((int16_t)buffer[2]) << 8) | buffer[3];
    accelRaw.x = (((int16_t)buffer[4]) << 8) | buffer[5];
    gyroRaw.y = (((int16_t)buffer[8]) << 8) | buffer[9];
    gyroRaw.z = (((int16_t)buffer[10]) << 8) | buffer[11];
    gyroRaw.x = (((int16_t)buffer[12]) << 8) | buffer[13];
#else
    /* sensors step 2.1 read from buffer */
    accelRaw.y = (((int16_t)buffer[0]) << 8) | buffer[1];
    accelRaw.z = (((int16_t)buffer[2]) << 8) | buffer[3];
...
//     sensorData.gyro.x = (gyroRaw.y - gyroBias.y) * SENSORS_DEG_PER_LSB_CFG;
// #else
//     sensorData.gyro.x = (gyroRaw.y - gyroBias.y) * SENSORS_DEG_PER_LSB_CFG;
// #endif

//     sensorData.gyro.y = (gyroRaw.z - gyroBias.z) * SENSORS_DEG_PER_LSB_CFG;
//     sensorData.gyro.z = (gyroRaw.x - gyroBias.x) * SENSORS_DEG_PER_LSB_CFG;
//     /* sensors step 2.5 low pass filter */
//     applyAxis3fLpf((lpf2pData *)(&gyroLpf), &sensorData.gyro);

// #ifdef CONFIG_TARGET_ESPLANE_V1
//     //accScaled.x = (accelRaw.x) * SENSORS_G_PER_LSB_CFG / accScale;
//     accScaled.x = (accelRaw.y) * SENSORS_G_PER_LSB_CFG / accScale;
// #else
//     accScaled.x = (accelRaw.y) * SENSORS_G_PER_LSB_CFG / accScale;   
// #endif

//     accScaled.y = (accelRaw.z) * SENSORS_G_PER_LSB_CFG / accScale;
//     accScaled.z = (accelRaw.x) * SENSORS_G_PER_LSB_CFG / accScale;
