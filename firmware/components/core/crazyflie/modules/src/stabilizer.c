...
static void testProps(sensorData_t *sensors)
{
  ...
  if (testState == configureAcc)
  {
    ...
    minSingleLoadedVoltage[MOTOR_BLDC1] = minLoadedVoltage;
    minSingleLoadedVoltage[MOTOR_BLDC2] = minLoadedVoltage;
    minSingleLoadedVoltage[SERVO1] = minLoadedVoltage;
    minSingleLoadedVoltage[SERVO2] = minLoadedVoltage;
  }
  ...
  else if (testState == testBattery)
  {
    ...
    if (i == 1)
    {
      motorsSetRatio(MOTOR_BLDC1, 0xFFFF);
      motorsSetRatio(MOTOR_BLDC2, 0xFFFF);
      motorsSetRatio(SERVO1, 0xFFFF);
      motorsSetRatio(SERVO2, 0xFFFF);
    }
    else if (i < 50)
    {
...
//                  (double)minSingleLoadedVoltage[MOTOR_M4]);
      DEBUG_PRINTI("%f %f %f %f %f %f\n", (double)idleVoltage,
                  ...
                  (double)(idleVoltage - minSingleLoadedVoltage[MOTOR_BLDC1]),
                  (double)(idleVoltage - minSingleLoadedVoltage[MOTOR_BLDC2]),
                  (double)(idleVoltage - minSingleLoadedVoltage[SERVO1]),
                  (double)(idleVoltage - minSingleLoadedVoltage[SERVO2]));
