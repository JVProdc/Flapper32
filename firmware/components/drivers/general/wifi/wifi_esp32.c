...
//             memcpy(inPacket.data, rx_buffer, len);
//             cksum = inPacket.data[len - 1]; // Extract checksum byte
//             inPacket.size = len - 1;       // Exclude checksum

//             uint8_t calculated_cksum = calculate_cksum(inPacket.data, len - 1);
//             // DEBUG_PRINT_LOCAL("Received checksum: %02X, Calculated checksum: %02X", cksum, calculated_cksum);

//             if (cksum == calculated_cksum && inPacket.size < 64) {
//                 xQueueSend(udpDataRx, &inPacket, M2T(2));
//                 if (!isUDPConnected) isUDPConnected = true;
//             } else {
//                 DEBUG_PRINT_LOCAL("udp packet cksum unmatched (received: %02X, calculated: %02X)", cksum, calculated_cksum);
//             }
...
void wifiInit(void)
{
    ...
    sprintf(WIFI_SSID, "%s", CONFIG_WIFI_BASE_SSID);
